...
pub struct LdrsConfig {
    ...
    #[serde(default)]
    dest: Option<String>,
    tables: Vec<serde_yaml::Value>,
}

pub fn get_runtime_from_env() -> Result<LdrsRuntimeContext, anyhow::Error> {
    let src_url = std::env::var("LDRS_SRC").map_err(|_| anyhow::Error::msg("LDRS_SRC not set"))?;
    let dest_url =
        std::env::var("LDRS_DEST").map_err(|_| anyhow::Error::msg("LDRS_DEST not set"))?;
    Ok(LdrsRuntimeContext { src_url, dest_url })
}
...
pub fn get_parsed_config(
    ...
    let src = value
        ...
        .transpose()
        .map_err(|e| anyhow::Error::msg("src is not correctly set in the table block"))?
    ...
    let dest = value
        ...
        .map(|v| String::deserialize(v))
        .transpose()
        .map_err(|e| anyhow::Error::msg("dest is not correctly set in the table block"))?
