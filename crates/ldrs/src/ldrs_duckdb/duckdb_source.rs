...
#[serde(tag = "src")]
#[schemars(
    description = "DuckDB source. Spawns the `duckdb` CLI and reads its Arrow IPC output. Requires duckdb on PATH (or LDRS_DUCKDB_BIN) with the `nanoarrow` community extension installed. Both modes get `-bail`, `.output /dev/null`, `LOAD nanoarrow`, the Arrow COPY wrapper, `pre_sql`, `{{ src_url }}`, and the libpq vars derived from a `postgres://` URL. Managed mode adds `-init /dev/null`, `SET TimeZone='UTC'`, and a `CREATE SECRET` or Postgres `ATTACH` derived from the src URL. `duckdb.raw` adds none of those, so your init file, extensions and stored secrets apply: use it when credential_chain cannot express the auth, or the session is already configured. Raw emits no `ATTACH`, so a `postgres://` URL leaves `pg.<schema>.<table>` an unknown catalog until you attach it in `pre_sql`."
)]
